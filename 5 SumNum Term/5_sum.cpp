		...
		while(!terms_and_nodes.eof()&&stopflag)
		{
			...
			if(s.length()>0)
			{
				if(s[0]=='n')//нашли новый маршрут
				{
					...
					str.str("");
					str<<root_path<<points[point_num]<<delim<<"res"<<type_str[type]<<"$"<<order<<"$"<<subOrder<<"$"<<cur_num<<"_"<<points[point_num]<<".txt";
					if(res_file)
					{
						res_file.close();
					} 
					...
					while(!res_file.eof())
					{
						...
						if(s.length()>0)
						{
							...
							if (if_cur_term)
							{
								...
								if ((a_operators==2)&&(s_length==cur_term.Sop.size())&&cur_term.Sop[0].n ==  0)
								{
									int flag=true;
